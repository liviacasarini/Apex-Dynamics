...
export default function EstrategiaTab({ activeProfile, vehicleType = 'car' }) {
  const C = useColors();
        ...
        <Row>
          <Field label="Velocidade pit lane" unit="km/h" C={C} half>
            <input type="number" value={d.pitSpeedLimit} onChange={set('pitSpeedLimit')}
              placeholder={(() => { const at = readActiveTrack(profileId); return at?.pitSpeedLimit ? `${at.pitSpeedLimit} (Pistas)` : 'Ex: 60'; })()}  /* unif: limite igual p/ ambos */
              style={IB} />
          </Field>
